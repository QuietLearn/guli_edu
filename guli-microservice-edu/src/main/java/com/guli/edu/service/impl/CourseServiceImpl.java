...
public class CourseServiceImpl extends ServiceImpl<CourseMapper, Course> implements CourseService {
    ...
    //todo 做类似众筹网的项目提交的几个步骤, 当然需要做用户登录校验拦截器，没有登录无法访问接口，前端vue对没有登录的访问已经做了拦截
    // 可以读取cookie[cookie方便，只要浏览器访问就自动带给服务器，不用前端自己控制携带token]
    // 不对redis的user做expire延期，设置为7天，过期后需要重新登录
    // 做真正的用户登录服务，然后存入redis缓存
     ...
     */
    @Override
    public Map<String, Object> pageListWeb(Page<Course> pageParam,CourseQuery courseQuery) {
        QueryWrapper<Course> queryWrapper = new QueryWrapper<>();
     ...
     *
     */
    public ServerResponse getCourseDetail(String courseId){
        //todo 更新课程浏览数,同ip最好限制住
        Course course = baseMapper.selectById(courseId);
        course.setViewCount(course.getViewCount() + 1);
        baseMapper.updateById(course);

        CourseDetailVo courseDetailVo = baseMapper.selectCourseDetailVoById(courseId);
...
//        不要让 不同的业务里面的貌似相同的方法让它通用
        List<ChapterVo> chapterVoList = (List<ChapterVo>) chapterService.getChildItemsByCourseId(courseId).getData();

        courseDetailVo.setChapterVoList(chapterVoList);

        return ServerResponse.createBySuccess("查询成功",courseDetailVo);
    }
